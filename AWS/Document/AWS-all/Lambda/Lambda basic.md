...
- Provisioned Concurrency trong AWS Lambda là một tính năng cho phép bạn duy trì một số lượng phiên bản Lambda đã sẵn sàng để xử lý các yêu cầu ngay lập tức. Điều này giúp giảm thiểu độ trễ `cold start`, một vấn đề thường gặp khi Lambda phải khởi động một phiên bản mới để xử lý yêu cầu.
...
- Các điểm chính về Provisioned Concurrency:
  ...
  + Cost: Provisioned Concurrency tính phí dựa trên số lượng phiên bản bạn đã cấu hình và thời gian mà chúng được duy trì. Điều này có thể làm tăng chi phí so với việc sử dụng Lambda chỉ trong chế độ tự động.
